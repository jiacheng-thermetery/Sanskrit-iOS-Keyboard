...
final class KeyboardViewController: UIInputViewController {
    ...
    private let transliterator = IASTToDevanagariTransliterator()
    ...
    /// Cursor moved or external edit — drop any pending state so we don't
    /// try to delete characters that no longer exist where we expect.
    override func selectionWillChange(_ textInput: UITextInput?) {
        super.selectionWillChange(textInput)
        transliterator.reset()
    }

    private func apply(_ edit: IASTToDevanagariTransliterator.Edit) {
